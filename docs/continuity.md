...
Source-specific discovery, grammar, and authority stay outside
`pkg/convergence`. Continuity observations compose into the existing
source-neutral admission judgment and scheduler; they do not create a second
planner or scheduler, and they do not require Hive-created beads. The current
configured project and policy boundary is one adoption boundary, not an
assumption that a single issue-filter label is the only future boundary.
